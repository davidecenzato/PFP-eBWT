/*
 * Code to build the the runlength eBWT and the GCA of a string collections.
 * 
 * This code is adapted from https://github.com/maxrossi91/pfp-thresholds/blob/master/include/pfp/pfp_thresholds.hpp
 */

#ifndef PFP_EBWT_GCA_COMPLETE_HPP
#define PFP_EBWT_GCA_COMPLETE_HPP
...
public:
    ...
    {
        // print GCA sample
        update_sa_sample(curr, std::get<0>(occ));
        print_sa();
    ...
    // write a GCA entry to file
    inline void print_sa()
    {
        if (fwrite(&sas, bytes_gca, 1, ebwt_file_gca) != 1)
            error("GCA write error");
        // document array entry
        if (fwrite(&dsa, bytes_da, 1, ebwt_file_da) != 1)
            error("DA write error");
    }
    ...
    inline void update_sa_sample(phrase_suffix_t &curr, size_t pos)
    { 
    ...
    pfp_ebwt_gca_complete(pfp_parse_gca &p_, dictionary &d_, std::string filename_, size_t w_, bool rle_): 
        pars(p_),
        dict(d_),
        filename(filename_),
        w(w_),
        ...
        head(0)
    {
        ...
        if((I_file = fopen(outfile.c_str(), "w")) == nullptr)
            error("open() file " + outfile + " failed");
        // GCA files
        outfile = filename + std::string(".gca"); 
        if((ebwt_file_gca = fopen(outfile.c_str(), "w")) == nullptr)
            error("open() file " + outfile + " failed");
        outfile = filename + std::string(".da"); 
        if((ebwt_file_da = fopen(outfile.c_str(), "w")) == nullptr)
            error("open() file " + outfile + " failed");
        ...
        info += "Number of bytes per GCA conjugate offset (.gca file): " + std::to_string(bytes_gca) + "\n";
        info += "Number of bytes per GCA sequence offset (.da file): " + std::to_string(bytes_da) + "\n";
...
#endif /* PFP_EBWT_GCA_COMPLETE_HPP */