...
void *cyclic_mt_parse_fasta(void *dx)
{
  ...
  uint64_t pos = 0, first_pos = 0, offset = 1, first_offset = 1;
      ...
      if(is_valid_base(c)){
          ...
          uint64_t hash = krw.addchar(c);
            if (hash%arg->p==0 && krw.current == arg->w){
                if(first_trigger==0){
                    first_trigger = 1;
                    first_pos = pos = fword.size() - arg->w;
                    first_offset = offset = first_pos + 1;
                    word.erase(0,word.size() - arg->w);
                }
                else{
                    save_update_word(word,arg->w,*wordFreq,d->parse,d->last,d->sa,d->o,pos,offset,0);
                    d->words++;
                }
            }
          pc = c;
        ...
        if(c == EOF || c == '>'){
            ...
            string final_word = word + fword.erase(0,arg->w - 1);
            pos = first_pos;
            offset = first_offset;
            save_update_word(final_word,arg->w,*wordFreq,d->parse,d->last,d->sa,d->o,pos,offset,1);
            d->words++;
            krw.reset();
            first_trigger = 0;
...
uint64_t cyclic_mt_process_file_fasta(Args& arg, map<uint64_t,word_stats>& wf)
{
  ...
  cout << "nome file: " << arg.inputFileName << endl;
  ...
  while ( ((c = fgetc(fp)) != EOF) ) {
    ...
    if(current_pos == th_sts[j] && j < arg.th + 1 && true_end > true_start){
        ...
        td[j-1].o = open_aux_file_num(arg.inputFileName.c_str(),EXTOFF,j-1,"wb");
